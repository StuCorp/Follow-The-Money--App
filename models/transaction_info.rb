...
def self.sql_store
  return "SELECT users.id AS user_id, users.name AS user_name, transactions.buy_date AS buy_date, transactions.id AS transaction_id, transactions.name AS transaction_name, transactions.cost AS transaction_cost, tags.name AS tag_name, providers.id AS provider_id, providers.name AS provider_name, transactions.luxury as transaction_luxury FROM users INNER JOIN transactions ON users.id = transactions.user_id INNER JOIN tags ON transactions.tag_id = tags.id INNER JOIN providers ON transactions.provider_id = providers.id"
end

  def self.full_info
    sql = sql_store + " ORDER BY buy_date DESC;"
    results = SqlRunner.run(sql)
    return results.map {|result| TransactionInfo.new(result) }
  end

  def self.full_info_sort_by(column_name, desc_or_asc)
    sql = sql_store + " ORDER BY #{column_name} #{desc_or_asc};"
    results = SqlRunner.run(sql)
    return results.map {|result| TransactionInfo.new(result) }
  end
...
def self.find_id(id)
  sql = sql_store + " WHERE transactions.id = #{id};"
  result = SqlRunner.run(sql).first
  return TransactionInfo.new(result)
end

def self.full_info_by_tag
  sql = sql_store + " ORDER BY tag_name, buy_date DESC;"
  results = SqlRunner.run(sql)
  return results.map {|result| TransactionInfo.new(result) }
end

def self.full_info_by_provider
  sql = sql_store + " ORDER BY provider_name, buy_date DESC;"
  results = SqlRunner.run(sql)
  return results.map {|result| TransactionInfo.new(result) }
end
...
def self.full_info_current_month
  ...
  sql = sql_store + "  WHERE buy_date BETWEEN '#{this_month}' AND CURRENT_DATE ORDER BY buy_date DESC;"
  results = SqlRunner.run(sql)
  return results.map {|result| TransactionInfo.new(result) }
end

def self.full_info_by_provider_current_month
  this_month = TransactionInfo.start_of_this_month()
  sql = sql_store + " WHERE buy_date BETWEEN '#{this_month}' AND CURRENT_DATE ORDER BY provider_name, buy_date DESC;"
  results = SqlRunner.run(sql)
  return results.map {|result| TransactionInfo.new(result) }
end
...
def self.full_info_current_month_sort_by(column_name, desc_or_asc)
  this_month = TransactionInfo.start_of_this_month()
  sql = sql_store + " WHERE buy_date BETWEEN '#{this_month}' AND CURRENT_DATE ORDER BY #{column_name} #{desc_or_asc};"
  results = SqlRunner.run(sql)
  return results.map {|result| TransactionInfo.new(result) }
end
